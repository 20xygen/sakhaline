...
// function get_cords() {
    ...
    function init() {
        ...
        ymaps.route([
            ...
            for (var i = 0; i < route.getPaths().getLength(); i++) {
                ...
                // console.log('i', i, 'Segments:', segments)

                for (var j = 0; j < segments.length; j++) {
                    var street = segments[j].getStreet();
                    moveList += ('Едем ' + segments[j].getHumanAction() + (street ? ' на ' + street : '') + ', проезжаем ' + segments[j].getLength() + ' м.,');
                    moveList += '</br>'
                    // console.log('Seg', j,  segments[j]['_geometry']['coordinates'])
                    // for (var k = 0; k < segments[j].length; k++) {
                        for (var l = 0; l < segments[j]['_geometry']['coordinates'].length; l++){
                            // console.log('Seg', j,  segments[j]['_geometry']['coordinates'[l]])
                            rout_x.push(segments[j]['_geometry']['coordinates'][l][0])
                            rout_y.push(segments[j]['_geometry']['coordinates'][l][1])
                        }
                    // }
                }
    ...
    // return [rout_x, rout_y]
// }

// console.log('Rout_x not in init:', rout_x)
// get_cords();
// calculate();
...
function calculate(){
    ...
    var test_x = [50.711203, 50.808101, 50.70407, 50.089067, 48.745811, 48.078602, 47.177426, 47.133443, 47.191803, 47.812813]
    var test_y = [141.43932, 142.777087, 143.706427, 144.282254, 143.626249, 144.282254, 143.961295, 142.716227, 142.112464, 141.455416]
    ...
    console.log('CORDS:', test_x, test_y)
    ...
    for (let i = 0; i < test_x.length - 1; i++) {
        next_x = test_x[i+1]
        next_y = test_y[i+1]
        cur_x = test_x[i]
        cur_y = test_y[i]
...
function reload_div() {
    ...
    document.getElementById('map').innerHTML = document.getElementById('map');
}